...
| `texture_strength` | `0.4` | Skin detail lifted from the operator's own source photo and warped onto the face each frame. **The fraction of the measured gap to close** — the compositor measures the real face's high-frequency energy in the same pixels, less what the swap and grain already carry, so `1.0` is parity and overshoot is impossible. Reaches `2.0` over the API for diagnosis only; the desktop slider stops at 1.0. **On at 0.4 since 2026-09-15**, judged on two live sessions over Route A's complexion grade; its earlier net-negative verdict was measured on a target-coloured face with the old donor picker. Linear from 0.2 up |
...
| `skin_complexion` | `1.0` | **Route A** (RESEMBLANCE.md), **final, on by default** — three footage runs on the hardest pairing, the last (2026-09-15) *"done and fully acceptable"*; the neck under a fair-on-dark pairing in bad light is the accepted limitation, and real pairings sit far from it. Grades every visible skin pixel of the *target* frame — face, neck, ears, chest, hands — toward the source's complexion **before** the swap, as the fraction of the measured gap to close, so `_match_color` then pulls the swapped face toward surroundings that already carry the source's tone and the face and the neck agree. Chroma shifted, lightness scaled (shading ratios survive), both bounded, smoothed on its parameters. Set `SKIN_COMPLEXION=0` to turn it off |
| `skin_harmonise` | `0.7` | Route A's second pass: after the grade, body skin is corrected toward the graded face — chroma fully, lightness toward a 0.82 neck-to-face floor, both bounded — so face and neck agree whatever left them apart. Built for the fair-on-dark neck; unjudged on footage. `0` off |
| `skin_complexion_hands` | `True` | Whether that grade reaches body skin outside the corridor below the face — hands and arms. A footage question, which is why it is a switch |
| `complexion_base` | `auto` | What Route A aims at: `auto` is the median of the source photographs; `mst01`–`mst10` is a Monk Skin Tone step the operator chose, with the photographs supplying undertone inside a bound — a prior, not a destination. Disagreement past the bound is said once. A COMPLEXION dropdown in the sidebar under RESTORATION sets it, each step drawn as its swatch |
| `complexion_target_base` | `auto` | The **target's** own tone as a Monk step — the operator's. With both tones declared, the ratio of the two swatches is complexion with the room cancelled out and licenses a lightness gain up to 2.5×; with either `auto` the stage stays in [0.80, 1.25], because photographs-against-frame is two rooms. Measured necessary on a fair-on-dark pairing where the narrow cap pinned on every frame. MY TONE dropdown under COMPLEXION |
| `skin_model` | `seeded` | Which skin segmentation backend finds skin outside the face. `seeded` needs no model file and knows colour, not anatomy — a skin-coloured wall is read as skin |
...
- `pipeline/services/complexion.py`: the **other** axis the cosine is blind to — whether the output's skin is the source's colour or the target's, in LAB chroma against a median-of-photographs source reference. `complexion_gap` is the pairing, `complexion_face` is the number to drive down, `complexion_target` the leakage direction, `complexion_seam` the colour step at the jaw a face-only transfer creates. Same interval as the identity probe. See RESEMBLANCE.md §3.1
- `pipeline/services/skin.py`: `SkinSegmenter` — where the person's skin is *outside* the face: neck, ears, chest, hands. A registry; the `seeded` backend needs no model file and fits a colour model per frame from the face's own skin. Knows colour, not anatomy — **a skin-coloured wall is read as skin, and it moves `complexion_neck`**. `SKIN_MODEL=`. See RESEMBLANCE.md §3.2

...
| `skin_complexion` | `0.0` | **Route A** (RESEMBLANCE.md). Grades every visible skin pixel of the *target* frame — face, neck, ears, chest, hands — toward the source's complexion **before** the swap, as the fraction of the measured gap to close, so `_match_color` then pulls the swapped face toward surroundings that already carry the source's tone and the face and the neck agree. Chroma shifted, lightness scaled (shading ratios survive), both bounded, smoothed on its parameters. **Off by default, never judged on footage** |
